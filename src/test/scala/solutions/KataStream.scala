...
class KataStream extends FunSuite {
  ...
  test("filter Stream for SACRAMENTO") {
    ...
    //  HINT
    //  begin
    val sacramentos: List[Estate] = estates.filter(estate => estate.city == "SACRAMENTO").toList
    //  end
  ...
  test("filter Stream for estates with price < 40000") {
    ...
    //  HINT
    //  begin
    val cheap: List[Estate] = estates.filter(estate => estate.price < 40000).toList
    //  end
  ...
  test("find most west estate") {
    ...
    //  HINT
    //  begin
    val mostWest: Estate = estates.sortBy(estate => estate.longitude).head
    //  end
  ...
  test("find 5 estate with lowest price for sqr ft") {
    ...
    //  HINT
    //  begin
    val cheapest: List[Tuple2[Estate, Int]] = estates.toStream
      .filter(estate => estate.sq__ft > 0 && estate.price > 0)
      .map(estate => (estate, estate.price / estate.sq__ft))
      .sortBy(_._2).take(5).toList
    //  end
  ...
  test("find estate with highest price for sqr ft") {
    ...
    val cheapest: List[Tuple2[Estate, Int]] = estates.toStream
      .filter(estate => estate.sq__ft > 0 && estate.price > 0)
      .map(estate => (estate, estate.price / estate.sq__ft))
      .sortBy(_._2).reverse.take(5).toList
    //  end
  ...
  test("count offers in each city, sorted desc") {
    ...
    //  HINT
    //  begin
    val collected: List[(String, Int)] = estates.toStream
      .groupBy(estate => estate.city)
  ...
  test("get list with average price for sqr ft in each city") {
    ...
    val collected: List[(String, Int)] = estates.toStream
      .groupBy(estate => estate.city)
      .map(x => (x._1, x._2.filter(estate => estate.sq__ft > 0)
        .map(estate => estate.price / estate.sq__ft)
        .toList.sum / x._2.size))
      .toList
      .sortBy(_._2)
    //  end
  ...
  test("get list with average price for sqr meter for each zipcode (zip, city, price/sqr_ft), sorted by price/sqr_ft, only with places where avg price > 0") {
    ...
    //  HINT
    //  begin
    val collected: List[(String, String, Int)] = estates.toStream
      .groupBy(estate => estate.zip)
      .map(x => (x._1, x._2.filter(_.zip == x._1).head.city, x._2.filter(estate => estate.sq__ft > 0)
        .map(estate => estate.price / estate.sq__ft)
        .toList.sum / x._2.size))
      .toList
      .filter(_._3 > 0)
  ...
  test("count flattypes, grouped by city (city, flattype, number_of_beds), sorted desc by number of beds") {
    ...
    //  HINT .flatMap
    //  begin
    val grouped: List[(String, String, Int)] = estates
      .groupBy(_.city)
      .map(x => (x._1, x._2.groupBy(e => e.flat_type)))
      .flatMap(x => x._2.map(y => (x._1, y._1, y._2.size)))
      .toList
      .sortBy(_._3)
      .reverse
    //  end
  ...
  test("get max distance between estates in each city") {
    ...
    //  begin
    val groupByCities: List[(String, Double)] = estates
      .groupBy(_.city)
      .map(v => (v._1, v._2
        .map(e1 => v._2
          .map(e2 => calc_distance(e1, e2))
          .max)
        .max))
      .toList
      .sortBy(_._2)
  ...
  test("get middle point in each city") {
    ...
    //  HINT
    //  begin
    val groupByCities: List[(String, (Double, Double))] = estates
      .groupBy(_.city)
      .map(v => (v._1, (v._2.map(e => e.longitude).sum / v._2.size, v._2.map(e => e.latitude).sum / v._2.size)))
      .toList
    //  end
    ...
    assert(groupByCities(38)._1 == "RANCHO MURIETA")
    assert(groupByCities(38)._2._1 == -121.08746466666666)
    assert(groupByCities(38)._2._2 == 38.49742966666667)
  }
  ...
  test("count beds in each city") {
    ...
    //  HINT
    //  begin
    val groupByCities: Map[String, Int] = estates
      .groupBy(_.city)
      .map(x => (x._1, x._2.map(e => e.beds).sum))
    //  end

    assert(groupByCities.size == 39)
    assert(groupByCities.get("SACRAMENTO").get == 1329)
    assert(groupByCities.get("CARMICHAEL").get == 61)
  }
  ...
  test("count avg beds in each city, and take top 3 with highest avg") {
    ...
    //  HINT  .toDouble
    //  begin
    val citiesWithBeds: List[(String, Double)] = estates.groupBy(_.city)
      .map(estates => (estates._1, estates._2.map(_.beds).sum.toDouble / estates._2.size.toDouble))
      .toList
      .sortBy(_._2)
      .reverse
      .take(3)
    ...
    assert(citiesWithBeds.size == 3)
    assert(citiesWithBeds(0)._1 == "GRANITE BAY")
    assert(citiesWithBeds(0)._2 == 4.666666666666667)
    assert(citiesWithBeds(1)._1 == "LOOMIS")
    assert(citiesWithBeds(1)._2 == 4.0)
    assert(citiesWithBeds(2)._1 == "EL DORADO HILLS")
    assert(citiesWithBeds(2)._2 == 3.4347826086956523)
  }
  ...
  test("in which 5 cities is highest average price") {
    ...
    //  HINT
    //  begin
    val expensiveCities: List[(String, Double)] = estates.groupBy(_.city)
      .toList
      .map(estate => (estate._1, estate._2.map(_.price).sum.toDouble / estate._2.size))
      .sortBy(_._2)
      .reverse
      .take(5)
    //  end
  ...
  test("what is smallest price for 2 baths and 2 beds") {
    ...
    //  HINT
    //  begin
    val cheapest: Estate = estates.filter(estate => estate.beds > 2 & estate.baths > 2)
      .sortBy(_.price)
      .head
    //  end
  ...
  test("get number of zipcodes for each city") {
    ...
    //  HINT
    //  begin
    val cities_zips: List[(String, Int)] = estates.groupBy(_.city)
      .toList
      .map(estate => (estate._1, estate._2.map(_.zip).distinct.size))
      .sortBy(_._2)
      .reverse
      .toList
    //  end
  ...
  test("find what type of estate is cheapest") {
    ...
    //  HINT
    //  begin
    val flat_types: List[(String, Double)] = estates.groupBy(_.flat_type)
      .map(x => (x._1, x._2.map(_.price).sum.toDouble / x._2.size))
      .toList
      .sortBy(_._2)
    //  end
  ...
  test("find if there are any duplicates in addresses, sorted by alphabeticall order") {
    val estates: List[String] = getEstatesList.map(estate => ("%s %s".format(estate.city, estate.street)))

    //  HINT groupBy(identity)
    //  begin
    val addresses: List[(String, Int)] = estates
      .groupBy(identity)
      .mapValues(_.size)
      .toList
      .filter(_._2 > 1)
    ...
    assert(addresses(0)._1 == "CITRUS HEIGHTS 8306 CURLEW CT")
    assert(addresses(0)._2 == 2)
    assert(addresses(1)._1 == "LINCOLN 7 CRYSTALWOOD CIR")
    assert(addresses(2)._1 == "SACRAMENTO 1223 LAMBERTON CIR")
    assert(addresses(3)._1 == "SACRAMENTO 4734 14TH AVE")
  }
  ...
  test("find if there are any duplicates in street names") {
    ...
    //  HINT
    //  begin
    val duplicated_streets: List[(String, Int)] = estates
      .groupBy(identity)
      .mapValues(_.size)
      .toList
      .filter(_._2 > 1)
